...
import { Edit2, Plus, X, Trash2, Globe, Type } from "lucide-react";
export const PaisesTable = () => {
  ...
  const cargarPaises = async () => {
    try {
      ...
      console.error("Error al cargar pa\xEDses:", err);
    }
  };
  const handleDelete = async (id, nombre) => {
    if (window.confirm(`\xBFSeguro que deseas eliminar el pa\xEDs "${nombre}"?`)) {
      try {
        await api.delete("/pais/delete", {
          data: { id }
        });
        alert("Pa\xEDs eliminado correctamente.");
        cargarPaises();
      } catch (err) {
        console.error("Error al eliminar pa\xEDs:", err);
        alert("No se pudo eliminar el pa\xEDs.");
      }
    }
  };
  const handleEdit = (pais) => {
    setEditingId(pais.id);
    setFormNombre(pais.nombre || "");
    setIsModalOpen(true);
  };
  const handleNew = () => {
    setEditingId(null);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/pais/${editingId}`, payload);
        alert("Pa\xEDs modificado correctamente (Endpoint PUT listo).");
      } else {
        await api.post("/pais", payload);
        alert("Pa\xEDs guardado correctamente!");
      }
      setIsModalOpen(false);
      cargarPaises();
    } catch (err) {
      console.error("Error al procesar pa\xEDs:", err);
      alert("Error en la solicitud. Verifica el nombre del pa\xEDs.");
    }
  };
  return <div className="space-y-4">
    ...
    /* SECCIÓN ENCABEZADO */
      ...
      <div className="flex justify-between items-center bg-surface/50 backdrop-blur-md p-4 border border-white/5 rounded">
        <div>
          <h3 className="text-sm font-bold text-white tracking-wide font-mono">Gestión Geográfica (Países)</h3>
          <p className="text-xs text-gray-500">Regiones globales utilizadas por Editoriales y Desarrolladores.</p>
        </div>
        <button
    onClick={handleNew}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-white/5 text-gray-400 font-mono border-b border border-white/5 uppercase tracking-wider text-[10px]">
              <th className="p-3 font-bold">ID País (UUID)</th>
              <th className="p-3 font-bold">Nombre del País</th>
          ...
          <tbody className="divide-y divide-white/5 text-gray-300 font-mono">
            {paises.length === 0 ? <tr>
                <td colSpan={3} className="p-8 text-center text-gray-600 italic">
              ...
              </tr> : paises.map((p) => <tr key={p.id} className="hover:bg-white/10 transition">
                  ...
                  <td className="p-3 font-sans font-medium text-white flex items-center gap-2">
                    <Globe size={12} className="text-gray-500" /> {p.nombre}
                  </td>
  ...
  >
          ...
          <div className="bg-surface/50 backdrop-blur-md border border-white/5 w-full max-w-md rounded shadow-2xl overflow-hidden font-sans">
            ...
            <div className="bg-white/5 px-4 py-3 border-b border border-white/5 flex justify-between items-center">
              <span className="text-xs font-mono font-bold tracking-wider text-blue-400 uppercase">
                {editingId ? "Modificar Pa\xEDs" : "Registrar Nuevo Pa\xEDs"}
              </span>
    ...
    /* Campo: Nombre País */
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-mono tracking-wide uppercase text-gray-400 flex items-center gap-1">
                  <Type size={12} className="text-blue-400" /> Nombre Oficial *
                </label>
                <input
    type="text"
    required
    ...
    placeholder="Ej. Estados Unidos, Japón, Argentina..."
